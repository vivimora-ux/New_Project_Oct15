...
// Allure report
import { readFile, rm } from "node:fs/promises"; // Archivo temporal remove
...
//const allureReporter = import('@wdio/allure-reporter').default
...
export const config = {
  ...
  // Define suites
  suites: {
    smoke: ["./tests/specs/home.test.js", "./tests/specs/contact.test.js"],
    components: ["./tests/specs/nav.test.js"],
  },
  ...
  capabilities: [
    {
      ...
      "goog:chromeOptions": {
        prefs: {
          "data.default_directory": join(__dirname, "/tests", "/data"),
        },
  ...
  baseUrl: "http://practice.sdetunicorns.com",
  //
   ...
   */
  // Optional: Add hooks to clean up the reports after each run
  afterTest: async function (test, context, result) {
    // Screenshot saving and attaching logic regardless of test outcome.
    const fileName = await getFileNameWithTimestamp(FILE_EXTENSIONS.PNG);
    try {
      ...
      addAttachment(`${fileName}`, screenShotData, "image/png");
